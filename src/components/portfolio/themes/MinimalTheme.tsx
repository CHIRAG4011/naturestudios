...
export function MinimalTheme({ portfolio }: ThemeProps) {
  ...
  const accent = portfolio.designConfig?.accentColor || '#38BDF8';

  return (
    <div className="min-h-screen bg-[#030712] text-[#F8FAFC] font-sans selection:bg-[#2563EB] selection:text-[#38BDF8]">
      {/* Top Masthead */}
      <header className="border-b border-[#0F1D38] py-8 px-6 lg:px-16 flex items-center justify-between">
        <div>
          <span className="text-[10px] font-mono tracking-[0.3em] uppercase text-[#94A3B8] block mb-1">
            CREATOR FOLIO // MINIMAL
          </span>
          <h1 className="text-xl font-bold uppercase tracking-tight text-[#F8FAFC]">
      ...
      {/* Hero */}
      <section className="px-6 lg:px-16 py-20 lg:py-28 max-w-5xl border-b border-[#0F1D38]">
        <div className="text-xs font-mono uppercase tracking-widest text-[#94A3B8] mb-6">
          {professionalIdentity.jobTitle || personalInfo.professionalTitle || 'Designer & Technologist'}
        </div>
        <h2 className="text-4xl sm:text-6xl lg:text-7xl font-medium tracking-tight text-[#F8FAFC] leading-[1.05] mb-8">
      ...
      {projects && projects.length > 0 && (
        <section className="px-6 lg:px-16 py-16 border-b border-[#0F1D38]">
          <div className="text-xs font-mono uppercase tracking-widest text-[#94A3B8] mb-8">
            01 / SELECTED WORK
          </div>
          <div className="divide-y divide-[#0F1D38]">
            {projects.map((proj, idx) => (
              <div key={proj.id || idx} className="py-8 group flex flex-col md:flex-row md:items-center justify-between gap-6 hover:bg-[#050B17] transition-colors px-4 -mx-4 rounded-xl">
                <div className="space-y-2 max-w-xl">
                  <div className="text-[10px] font-mono text-[#94A3B8] uppercase tracking-widest">
                    {proj.category} • {proj.client || 'STUDIO'}
                  </div>
                  <h3 className="text-2xl font-bold uppercase text-[#F8FAFC] group-hover:text-[#38BDF8] transition-colors flex items-center gap-2">
                ...
                {proj.thumbnail && (
                  <div className="relative w-full md:w-56 h-36 rounded-lg overflow-hidden border border-[#0F1D38] shrink-0">
      ...
      {skills && skills.length > 0 && (
        <section className="px-6 lg:px-16 py-16 border-b border-[#0F1D38]">
          ...
          <div className="flex flex-wrap gap-2.5">
            {skills.map((s, idx) => (
              <span key={idx} className="px-3.5 py-1.5 rounded-md border border-[#0F1D38] text-xs font-mono text-[#38BDF8]">
          ...
          <div className="space-y-6 max-w-3xl">
            {experience.map((exp, idx) => (
              <div key={idx} className="flex flex-col sm:flex-row sm:items-baseline justify-between gap-2 border-b border-[#1E0507] pb-6">
                <div>
                  <h4 className="text-base font-bold text-[#F8FAFC]">{exp.role}</h4>
                  <div className="text-xs font-mono text-[#38BDF8]">{exp.company}</div>
                </div>
      ...
      <footer className="py-12 px-6 lg:px-16 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs font-mono text-[#6E4249]">
        ...
        {personalInfo.publicEmail && (
          <a href={`mailto:${personalInfo.publicEmail}`} className="text-[#38BDF8] hover:underline">
            {personalInfo.publicEmail}
          </a>
        )}
      </footer>
