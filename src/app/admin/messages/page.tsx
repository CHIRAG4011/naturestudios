      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {loading ? (
          ...
          messages.map((m) => (
            <div
              key={m.id}
              className="p-5 rounded-2xl bg-[#070D1E] border border-[#172554] space-y-3 text-xs"
            >
              <div className="flex items-center justify-between">
                <span className="font-semibold text-[#F8FAFC]">{m.senderName || 'Visitor'}</span>
                <span className="text-[10px] font-mono text-[#94A3B8]">
                  {new Date(m.createdAt).toLocaleDateString()}
                </span>
              </div>
              <div className="font-mono text-[11px] text-[#38BDF8]">{m.senderEmail}</div>
              <p className="text-[#94A3B8] leading-relaxed">{m.content || m.message}</p>
            </div>
          ))
        )}
      </div>
