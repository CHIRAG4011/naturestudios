...
export async function POST(req: NextRequest) {
  ...
  const sanitizedTheme = {
    ...
    colors: {
      primary: String(theme.colors.primary || '#59171B'),
      secondary: String(theme.colors.secondary || '#FED7B8'),
      background: String(theme.colors.background || '#150304'),
      surface: String(theme.colors.surface || '#240709'),
      text: String(theme.colors.text || '#FFF5ED'),
      mutedText: String(theme.colors.mutedText || '#B89B8D'),
      border: String(theme.colors.border || '#3D0D13'),
      accent: String(theme.colors.accent || '#FED7B8'),
    ...
    gradients: {
      primary: String(theme.gradients?.primary || 'linear-gradient(135deg, #59171B 0%, #240709 100%)'),
      secondary: String(theme.gradients?.secondary || 'linear-gradient(135deg, #FED7B8 0%, #FFF5ED 100%)'),
      ambientMesh: String(theme.gradients?.ambientMesh || 'radial-gradient(circle, rgba(89, 23, 27, 0.4) 0%, transparent 70%)'),
    },
    typography: {
